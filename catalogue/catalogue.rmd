...
try(source("../R/ggpcapairs.r"), silent = T)
...
## Make radial tours manually from console
set.seed(20200903)
...
if(F) ## For generation see this file:
  file.edit("./catalogue/sim_mvnorm.r") ## Local only
## Load data
obj_nms <- c("sim_VII", "sim_EEE", "sim_VVV")
i_s <- 1:length(obj_nms)
for(i in i_s){
  try(load(file = paste0("./simulations/", obj_nms[i], ".rda")), silent = T)
  try(load(file = paste0("./catalogue/simulations/", obj_nms[i], ".rda")), silent = T)
}
