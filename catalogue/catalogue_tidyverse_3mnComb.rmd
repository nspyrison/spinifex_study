---
title: "Catalogue -- tidyverse simulations, (3 of 3, mean differences in combinations of variables)"
output:
  html_document:
    toc: true
    toc_depth: 3
...
```{r, initialize}
## SETUP MODELS, MEANS
models <- paste0(c("EEE", "EEV", "banana"), "_mnComb")
p      <- 4
k_cl   <- 3
cl_obs <- 140 ## banana_tform() expects evenly divisable by 5
...
assign(paste0("covs_", models[3]), 
       list("cl a" = .2 *  cov_circ,
            "cl b" = .05 * cov_circ, ## Apply banana_tform() to cl_b 
            "cl c" = cov_elipse_neg))
...
## ASSIGN GENERATED _mnComb SIMS
assign(models[1], rotate(this_sim(get(paste0("covs_", models[1])))))
assign(models[2], this_sim(get(paste0("covs_", models[2])))) ## need to banana_tform first.
assign(models[3], rotate(this_sim(get(paste0("covs_", models[3]))))) 
## Apply banana_tform() to cl b.
banana_mnComb[banana_mnComb$cl == "cl b", ] <-
  banana_tform(banana_mnComb[banana_mnComb$cl == "cl b", ])
banana_mnComb <- rotate(banana_mnComb)
...
design <- data.frame(
  ...
  `EEE family` = c("EEE", "EEE_corNoise", "EEE_mnComb"), 
  `EEV family` = c("EEV", "EEV_corNoise", "EEV_mnComb"),
  `banana family` = 
    c("banana", "banana_corNoise", "banana_mnComb"),
  check.names = FALSE
)
...
![](images/radialTour_tidyverse_banana_mnComb_mvar1.gif)
![](images/radialTour_tidyverse_banana_mnComb_mvar2.gif)
![](images/radialTour_tidyverse_banana_mnComb_mvar3.gif)
