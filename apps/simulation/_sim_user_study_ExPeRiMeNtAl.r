...
#### have been sourcedvlocally into sim_mvtnorm_cl.
...
sim_user_study <- function(cl_obs = 140,
  ...
  root <- paste0(here::here("apps/study/www/data"), "/")
  ...
  if(do_save == TRUE){
    ...
    rep_nms <- c(
      paste0("EEE_p4_0_1_t", 1:3),
      sapply(seq_len(length(in_nms)), function(i){
        paste0(in_nms[i], "_rep", 1:3)
      })
    )
    .save <- function(nm){save(get(nm, envir = globalenv()), file = paste0(root, nm, ".rda"))}
    sapply(seq_len(length(rep_nms)), function(i){
      .save(rep_nms[i])
    })
...
tpath_user_study <- function(do_save = FALSE){
  ...
  require("tourr")
  root    <- paste0(here::here("apps/study/www/data"), "/")
  in_nms  <- c("EEE_p4_0_1_t1", "EEE_p4_0_1_t2", "EEE_p4_0_1_t3",
               "EEE_p4_0_1_rep1", "EEE_p6_0_1_rep1",
               "EEE_p4_0_1_rep2", "EEE_p6_0_1_rep2",
               "EEE_p4_0_1_rep3", "EEE_p6_0_1_rep3")
