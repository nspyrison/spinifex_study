...
DO_REFRESH_FIGURES <- FALSE ## Causes sizable run time.
output_path <- "./output_sim_clSep/"

## Local decreasing Poisson func
myDecrPois <- function(len){
  out <- NULL
  for (i in 1:len)
    out <- c(out, rpois(1, i))
  return(rev(scales::rescale(out, to = c(-.3, .7))))
}

## Init for simulation
n_cl <- list(66, 33)
p <- 6
mns <- list(rpois(p, 1), rpois(p, 1))
diag_p <- diag(p)
diag(diag_p) <- rpois(p, 1)
...
cov1 <- cov2 <- diag_p
cov1[indx_ut] <- myDecrPois(len_ut)
cov1[indx_lt] <- t(cov1)[indx_lt]
cov2[indx_ut] <- myDecrPois(len_ut) 
cov2[indx_lt] <- t(cov2)[indx_lt]
covs <- list(cov1, cov2)

dat  <- sim_pDim_kCl(means = mns, sigmas = covs, n_points = n_cl)
