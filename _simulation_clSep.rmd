...
for (i in 1:k_cl){
  sub <- dat[clas == lvls[1], ]
  print(paste0("cluster#", i, ", level: ", lvls[i]))
  print("cluster column means:")
  kable((ls_cl_clmns[[i]] <- colMeans(sub)), digits = 2)
  print("cluster covariances:")
  (ls_cl_cov[[i]] <- cov(sub) )
  GGally::ggcorr(sub, label = TRUE, label_alpha = TRUE)
}

print(paste0("cluster#", i, ", level: ", lvls[i]))
print("CHANGE IN cluster column means:")
kable(ls_cl_clmns[[2]] - ls_cl_clmns[[1]], digits = 2)
print("CHANGE IN cluster covariances:")
...
![](output_sim_clSep/permute_clSep6.png)
