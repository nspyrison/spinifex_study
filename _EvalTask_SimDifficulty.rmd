...
Can we simulate the Cluster seperation that we expect? Let's create a simmulation that has variable contributions close to a target of: .8, .2, 0, 0, 0

```{r}
source('./R/sim_pDim_kCl.r')

mns <- list(c(8, 2, rep(0, 3)), rep(0, 5)) ## differences of 8, 2, 0, 0, 0
covs <- list(diag(5), diag(5))

set.seed(1234555)
sim8020 <- sim_pDim_kCl(means = mns, sigmas = covs)
#str(sim8020) ## cluster levels stored in attribute "cl_lvl". Try attr(sim8020, "cl_lvl")
