...
if(F)
  source("./R/ggproto_screeplot_ClSep.r");data=tourr::flea[, 1:6];class=tourr::flea$species;permute_var_num=1;n_reps=100;
rep_permute_var_ClSep <- function(data,
  ...
  for (i in 1:p){
    ...
    .tgt_df <- real_df_scree_ClSep[real_df_scree_ClSep$var %in% .tgt_var, ]
    .n <- nrow(.tgt_df)
    ...
    ## Add jitter'd points
    proto_perm_jitter[[i]] <-
      geom_jitter(aes(x = var, y = var_clSep[i]), .df, color = .cols[i],
                  alpha = .alp, shape = 3) 
    ## Add cross bar for 95% CI (picture box of a boxplot)
    proto_perm_jitter[[p + i]] <-
      geom_crossbar(aes(x = var, y = var_clSep[i], 
                        ymin = ci95_min, ymax = ci95_max), .df, 
  ...
  real_ggproto_screeplot_ClSep <- 
    ggproto_screeplot_ClSep(data, class, num_class_lvl_A,
                            num_class_lvl_B)
