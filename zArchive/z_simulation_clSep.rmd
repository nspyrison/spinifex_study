...
n_cl        <- list(50, 50, 400) ## address cluster c density, volume space
mns         <- list(c(-10, -10, 0, 0, 0), c(10, 10, 0, 0, 0), c(0, 0, 10, 0, 0))
p           <- length(mns[[1]])
k_cl        <- length(n_cl)
myCov       <- diag(p)
diag(myCov) <- c(1, 1, 3, 15^2, 10^2)
myCov[1, 2] <- myCov[2, 1] <- .7
covs        <- c(rep(list(myCov), 2), list(100 * diag(p)))
