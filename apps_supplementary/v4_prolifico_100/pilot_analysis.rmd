...
if(F){
  ...
  dat_active <- raw[raw$plot_active == TRUE, ] %>%
    filter(!is.na(key)) # Remove dummy rows with application notes
...
ub <- quantile(dat_task_agg$max_sec_to_resp, probs = .995) ## top 1/2 %
ggplot(dat_task_agg) + geom_density(aes(max_sec_to_resp, fill = "a"), alpha = .5) + my_theme +
  ggtitle("Time on page density") + 
  geom_vline(xintercept = c(lb, ub), linetype = 2L)

print("!! For the rest of the analysis we will be looking at the iddle chunk of data!!")
...
ggplot(dat_task_agg) + my_theme +
  ...
  ggtitle("Are disconnects causing people to sprint though?",
...
```{r}
my_ggplot(aes(x = factor, y = max_sec_on_pg,
              color = is_training, fill = is_training),
          "Seconds till last response by factor",
          .data = dat_qual) +
  coord_cartesian(ylim = c(0L, 70L))

my_ggplot(aes(x = eval, y = max_sec_on_pg,
              color = is_training, fill = is_training),
