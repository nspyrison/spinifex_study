...
my_ggpubr <- function(df, x, y, title = "Title missing"){
  ...
  .y_range <- diff(range(df[y]))
   #.y_offset_frac <- .1 * (length(my_comparisons) + 1)
  .y_offset_frac <-  .1 * (length(.fill_lvls) + 1)
  if(x=="order") .y_offset_frac <- .1
  .lab.y <- 1.25#.y_offset_frac * .y_range + max(df[y])

  ## Plot
  ggboxplot(df, x = x, y = y, fill = x, alpha = .6,
            palette = "Dark2", shape = x,
            add = "jitter", add.params = list(color = x, alpha = .3, width = .2)) +
    #stat_compare_means(comparisons = my_comparisons, label = "p.format") + ## pairwise test
    stat_compare_means(label = "p.signif", ref.group = .fill_lvls[1]) + ## Test each lvl w.r.t. first level.
    stat_compare_means(method = "anova", label.y = .lab.y, ## Global test
                       aes(label = paste0("", ..p.format..))) + # Add global anova p-value
    my_theme +
    ggtitle(title)
}
...
.model_comp_colnms <- c("Model name", "No. factors", "No. fixed terms", "AIC", 
...
if(interactive())
  compare_means(marks~location, data =dat_qual) ##seems large
```
...
## Not significant ##
(marks_by_data_viz_exp <- eval_survey %>% 
    rename(`data viz exp` = "data_viz_exp") %>% 
    my_ggpubr(
          x = "data viz exp", y = "marks",
          title = "Marks by data viz exp"))
# my_ggpubr(eval_survey,
