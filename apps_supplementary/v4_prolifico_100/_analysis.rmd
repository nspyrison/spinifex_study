...
if(F){
  ...
  dat_task_agg <- dat_task_agg %>%
    mutate(task_input_inter = 
             if_else(factor == "radial", task_input_inter -1L, task_input_inter))
...
perm_prolific_id_whitelist <- dat_qual %>%
  filter(`is even instance` == TRUE) %>%
  arrange(`parameter permutation`, removal_priority, desc(write_dt)) %>%
...
dat_qual <- dat_qual %>%
  filter(instance_id %in% perm_prolific_id_whitelist)
...
# lb <- quantile(dat_qual$`seconds to last response`, probs = .25) ## bottom 25 %
# ub <- quantile(dat_qual$`seconds to last response`, probs = .95) ## top 5 %
ann_offset <- 7
ggplot() +
...
# dat_qual <- dat_qual
#%>% filter(`seconds to last response` > lb, `seconds to last response` < ub)
...
## A qq plot of the Random Effectssimulation
...
model_comp_tbl <- tibble(`Model name` = names(model_ls),
                         `# of factors` = factors_vec,
                         `# of fixed effects` = fixef_vec,
                         `AIC` = round(.perf_df[, 1]),
                         `BIC` = round(.perf_df[, 2]),
...
marks_f_re_partcipant.data <- ## RE on participant and data
  lmer(marks ~ factor + (1 | participant) + (1 | simulation), data = dat_qual_SIMNM)
time_f_re_partcipant <- ## regress angainst 
  lmer(`seconds to last response` ~ factor + (1 | participant), data = dat_qual)
time_f_re_partcipant.data <-## RE on participant and data
...
model_comp_tbl2 <- tibble(`Model name` = names(model_ls2),
                          `# of factors` = factors_vec2,
                          `# of fixed effects` = fixef_vec2,
...
if(F){
  ...
  (figModel <- cowplot::plot_grid(resid, resim, ncol = 2, rel_widths = c(2, 3)))
