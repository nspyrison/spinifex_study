                      ...
                      .data = dat_qual)
{
  ggplot(.data, .aes) +
    ...
    my_theme +
    geom_point(
      position = position_jitterdodge(jitter.width = .3, jitter.height = .05), alpha = .3) +
    geom_boxplot(position = "dodge", alpha = .6)
}
## indirect (via ggpubr/cowplot) ggplot2 helpers

my_comparisons <- list( c("radial", "grand"), c("grand", "pca"), c("radial", "pca"))
my_ggpubr <- function(df, x, y, color = "factor", fill = "factor", title = "Title missing"){
  ggboxplot(df,  x = x, y = y, fill = fill, alpha = .6,
            palette = "Dark2",
            add = "jitter", add.params = list(color = color, alpha = .3, width = .2)) +
    stat_compare_means(comparisons = my_comparisons, label = "p.format") +
    stat_compare_means(label.y = 1.9) + 
    theme_minimal() +
    ggtitle(title)
}
legend_b_c <- get_legend(
  ggplot(dat_task_agg, aes(factor, marks, fill = marks)) + geom_point() + 
    theme_minimal() +
    theme(legend.box.margin = margin(0, 12, 0, 12),
          legend.position = "bottom",
          legend.justification = "center")
)
my_plot_grid_titles <- function(plot_grid_obj, title_char, subtitle_char, legend = legend_b_c){
  title <- ggdraw() + draw_label(title_char,fontface = 'bold', x = 0, hjust = 0)
  if(missing(subtitle) == TRUE){
      subtitle <- ggdraw() + 
    draw_label("Participant subjective measures",
               fontface = 'bold', x = 0, hjust = 0)
      return(cowplot::plot_grid(title, plot_grid_obj, legend,
                                ncol = 1,
                                rel_heights = c(.1, 1, .1)))
  }
  return(cowplot::plot_grid(title, subtitle, plot_grid_obj, legend,
                            ncol = 1,
...
m_f <- lmer(marks ~ factor + (1 | participant), data = eval_survey) ## Benchmark
m_f.demo.exp <-
  lmer(marks ~ factor + pronouns + age + education + task_understanding +
         data_viz_exp + analysis_exp + (1 | participant), data = eval_survey)
m_f.eval <-
  lmer(marks ~ factor + location + shape + dim + order + (1 | participant), data = eval_survey)
m_f.eval.demo.exp <-
  lmer(marks ~ factor + location + shape + dim + order + pronouns + age +
         education + task_understanding + data_viz_exp + analysis_exp +
         (1 | participant), data = eval_survey)
m_f_demo_exp <-
  lmer(marks ~ factor * (pronouns + age +
         education + task_understanding + data_viz_exp + analysis_exp) +
         (1 | participant), data = eval_survey)
m_f_eval_demo_exp <-
  lmer(marks ~ factor * (location + shape + dim + order + pronouns + age +
         ...
         (1 | participant), data = eval_survey)
m_sub.gt1 <-
  lmer(marks ~ factor + (location + shape + analysis_exp) + 
         dim + order + age + education + task_understanding +
         (1 | participant), data = eval_survey)
m_sub.gt2 <-
  lmer(marks ~ factor + (location + shape) + 
         dim + order + age + task_understanding +
         (1 | participant), data = eval_survey)
m_sub_gt1 <-
  lmer(marks ~ factor * (location + shape + analysis_exp) + 
         dim + order + age + education + task_understanding +
...
m_sub_gt1.re_sim <-
    lmer(marks ~ factor * (location + shape + analysis_exp) + 
         dim + order + age + education + task_understanding +
         (1 | participant)  + (1 | simulation), data = eval_survey)
m_sub_gt2 <-
  lmer(marks ~ factor * (location + shape) + 
         dim + order + age + task_understanding +
         (1 | participant), data = eval_survey)
...
model_ls3 <- list(`F` = m_f,
                  `F+demo+exp` = m_f.demo.exp,
                  `F+eval+demo+exp` = m_f.eval.demo.exp,
                  `F*demo*exp` = m_f_demo_exp,
                  `F*eval*demo*exp` = m_f_eval_demo_exp,
                  m_sub.gt1 = m_sub.gt1,
                  m_sub.gt2 = m_sub.gt2,
                  m_sub_gt1 = m_sub_gt1,
                  m_sub_gt2 = m_sub_gt2,
                  m_sub_gt1.re_sim = m_sub_gt1.re_sim,
                  m_sub_gt2.re_sim = m_sub_gt2.re_sim)
...
model_comp_tbl3 <- tibble(`Model name` = names(model_ls3),
                          `No. of factors` = factors_vec3,
                          `No. of fixed effects` = fixef_vec3,
                          `AIC` = round(.perf_df3[, 1]),
                          `BIC` = round(.perf_df3[, 2]),
                          `R2 (conditional on RE)` = round(.perf_df3[, 3], 3),
                          `R2 (marginal; w/o RE)` = round(.perf_df3[, 4], 3),
                          `RMSE` = round(.perf_df3[, 6], 3)
)
...
(marks_by_factor <- my_ggpubr(dat_qual, "factor", "marks", title = "Marks by factor"))
...
if(F){
  ## Save dat_qual, the df for the remaining 108 instance_id's. 
  ### NOW DONE IN LINE WITH if(interactive()) ###
  # saveRDS(dat_qual, "./apps_supplementary/v4_prolifico_100/data/dat_qual.rds")
  gg_eval_by_perm_num
  gg_marks_by_factor
  ...
  ggsave("./paper/figures/marksByFactor.png", marks_by_factor, 
