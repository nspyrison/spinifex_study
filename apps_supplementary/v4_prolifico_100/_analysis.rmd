...
my_ggpubr <- function(df, x = "factor", y = "marks", title = "Title missing", subtitle = waiver()){
  ...
  .lab.y <- .15 * (1 + .n_lvls) * .y_range + max(df[y])
...
ggEffectRanges <- plotREsim(REsim(ABC_re_parti_sim)) +
  ggtitle("Effect range vs mark quantiles", "Effect range for the model alpha * beta * gamma") +
...
## By participant
participant_agg <- dat_qual %>%
  dplyr::select(participant, marks) %>%
  group_by(participant) %>%
  summarise(y25 = quantile(marks, .25),
            y50 = quantile(marks, .5),
            y75 = quantile(marks, .75)) %>%
  ungroup() %>%
  mutate(participant = forcats::fct_reorder(participant, y50))
ggParticipantRanges <- ggplot(participant_agg) +
    geom_point(aes(participant, y50)) +
    geom_linerange(aes(participant, ymin = y25, ymax = y75)) +
    my_theme +
    theme(axis.text.x = element_blank(),
          axis.ticks.x = element_blank()) +
    coord_cartesian(ylim = c(-.75, .75)) +
    xlab("participant (n=6 every line)") +
    ylab("mark quantiles (25, 50, 75)")
ggParticipantRanges <- ggExtra::ggMarginal(ggParticipantRanges, 
                                      type = "density", fill = "grey80")
## By simulation
simulation_agg <- dat_qual %>%
  dplyr::select(simulation, marks) %>%
  group_by(simulation) %>%
  summarise(y25 = quantile(marks, .25),
            y50 = quantile(marks, .5),
            y75 = quantile(marks, .75)) %>%
  ungroup() %>% 
  mutate(simulation = forcats::fct_reorder(simulation, y50))
ggSimulationRanges <- ggplot(simulation_agg) +
    geom_point(aes(simulation, y50)) +
    geom_linerange(aes(simulation, ymin = y25, ymax = y75)) +
    my_theme + 
