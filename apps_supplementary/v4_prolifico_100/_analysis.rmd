...
#### EXPLORE ----
############################
ABC_coef <- summary(ABC)$coefficients %>% data.frame(check.names = F) %>% 
...
3) Select a model to examine it's coefficents.
...
############################
## The Models:
ta <-    lmer(`seconds to last response` ~ factor + (1 | participant) + (1 | simulation), data = dat_qual)
tab <-   lmer(`seconds to last response` ~ factor + location + (1 | participant) + (1 | simulation), data = dat_qual)
tac <-   lmer(`seconds to last response` ~ factor + shape + (1 | participant) + (1 | simulation), data = dat_qual)
tad <-   lmer(`seconds to last response` ~ factor + dim + (1 | participant) + (1 | simulation), data = dat_qual)
tabc <-  lmer(`seconds to last response` ~ factor + location + shape + (1 | participant) + (1 | simulation), data = dat_qual)
tabd <-  lmer(`seconds to last response` ~ factor + location + dim + (1 | participant) + (1 | simulation), data = dat_qual)
tacd <-  lmer(`seconds to last response` ~ factor + shape + dim + (1 | participant) + (1 | simulation), data = dat_qual)
tabcd <- lmer(`seconds to last response` ~ factor + location + shape + dim + (1 | participant) + (1 | simulation), data = dat_qual)
tAB <-   lmer(`seconds to last response` ~ factor * location + (1 | participant) + (1 | simulation), data = dat_qual)
tAC <-   lmer(`seconds to last response` ~ factor * shape + (1 | participant) + (1 | simulation), data = dat_qual)
tAD <-   lmer(`seconds to last response` ~ factor * dim + (1 | participant) + (1 | simulation), data = dat_qual)
tABC <-  lmer(`seconds to last response` ~ factor * location * shape + (1 | participant) + (1 | simulation), data = dat_qual)
...
#### EXPLORE ----
############################
t_ABC_coef <- summary(tABC)$coefficients %>% data.frame(check.names = F) %>% 
  mutate(" " = case_when(`Pr(>|t|)` < .001 ~ "***",
                         `Pr(>|t|)` < .01 ~ "**",
                         `Pr(>|t|)` < .05 ~ "*",
                         TRUE ~ ""))
...
if(F){
  ...
  ## Model table
  model_comp_tbl_ls <- list(model_comp_eval = model_comp_tbl)
  saveRDS(model_comp_tbl_ls, file = "./paper/figures/tblModelCompLs.rds")
  ## Selected model: ABC 
  saveRDS(ABC_coef, file = "./paper/figures/tblModelABCcoef.rds")
