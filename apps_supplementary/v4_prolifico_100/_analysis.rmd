...
coef <- summary(f_survey)$coefficients %>% data.frame() ## tibble failed
str(coef)

#coef %>% arrange(desc(abs(t.value))) ## Too messy, filter instead.
coef %>% filter(abs(t.value) > 1.5)
##TODO: continue from here; what are the next less than full models; those with terms with an lvl of t> 1.5??
```
