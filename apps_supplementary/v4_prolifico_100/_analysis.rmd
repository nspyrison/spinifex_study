...
my_ggpubr <- function(df, x = "factor", y = "marks", title = waiver(), subtitle = waiver(), facet = NULL){
  ...
  .df_n_obs <-
    data.frame(.x = 1:.n_lvls - .2,
               .y = min(df[y]),
               plyr::count(df[x]))
  .df_n_obs <- .df_n_obs %>%
    mutate(grp = .df_n_obs[x],
           lab = paste0(c("n=", rep("", .n_lvls - 1L)), freq, "\n"))
...
my_ggpubr_with_n <- function(df, x = "factor", y = "marks", title = waiver(), subtitle = waiver(), facet = NULL){
  .n_lvls <- df[x] %>% pull() %>% levels() %>% length()
.df_n_obs <-
  data.frame(.x = 1:.n_lvls - .2,
...
```{r}
.factor <- my_ggpubr_with_n(dat_qual, x = "factor", y = "marks")
.location <- my_ggpubr_with_n(dat_qual, x = "location", y = "marks")
.shape <- my_ggpubr_with_n(dat_qual, x = "shape", y = "marks")
.dim <- my_ggpubr_with_n(dat_qual, x = "dim", y = "marks")
...
top <- cowplot::plot_grid(.factor, .location, .shape, .dim, nrow= 1)
gc()
(violin_ABcd <- 
  cowplot::plot_grid(title, top, .FactorLocation + ggtitle("", ""), ncol = 1, rel_heights = c(.1,1,1)))
```
...
if(F){
  ...
  ggsave("figMarksABcd.png", violin_ABcd, path = "./paper/figures",
         device = "png", width = .w, height = .w, unit = .u)
}
```
