...
## Latex formatting
model_comp_tbl3 <- model_comp_tbl3 %>%
  format_column_extrema(4, min) %>%
  format_column_extrema(5, min) %>%
  format_column_extrema(6, max) %>%
  format_column_extrema(7, max) %>%
...
model_ls2 <- list(`marks~F+RE(parti.)` = marks_f_re_partcipant,
...
if(F){
  ...
  ## Model tables
  model_comp_tbl_ls <- list(model_comp_tbl, model_comp_tbl2, model_comp_tbl3)
