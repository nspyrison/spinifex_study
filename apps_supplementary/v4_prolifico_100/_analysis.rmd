...
## app_instance_agg
app_intance_agg <- dat_task_agg %>% 
  group_by(instance_id, participant_num, `parameter permutation`, participant, `is training`) %>% 
  summarise(`full evaluations` = max(sum(`is training`) / 3, sum(!`is training`) / 6)
  ) %>% ungroup() %>% 
  mutate(is_instance_training_even = if_else(`full evaluations` == 1, TRUE, FALSE)) %>% 
  arrange(desc(`full evaluations`))
## Find vector of evenly evaled instances
even_evaled_instance_ids <- app_intance_agg %>%
  filter(`is training` == TRUE, is_instance_training_even == TRUE) %>%
  pull(instance_id)
## Decode back to the original dataset.
dat_qual <- dat_task_agg %>% mutate(
  `is even eval` = if_else(instance_id %in% even_evaled_instance_ids, TRUE, FALSE))


## participant_agg
participant_agg <- dat_task_agg %>%
  group_by(participant, `is training`) %>%
  summarise(`full evaluations` = max(sum(`is training`) / 3, sum(!`is training`) / 6)
  ) %>% ungroup() %>% 
  arrange(desc(`full evaluations`))

## perm_num_agg
perm_num_agg <- dat_task_agg %>%
  group_by(full_perm_num, `is training`) %>%
  summarise(`full evaluations` = max(sum(`is training`) / 3, sum(!`is training`) / 6)
  ) %>% ungroup() %>% 
  arrange(desc(`full evaluations`))
...
ggplot(participant_agg, aes(x = participant, y = `full evaluations`, 
                         color = `is training`, fill = `is training`)) +
