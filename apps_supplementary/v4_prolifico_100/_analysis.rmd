...
my_ggpubr <- function(df, x = "factor", y = "marks", title = waiver(), subtitle = waiver(), facet = NULL){
  ...
  ggviolin(df, x = x, y = y, fill = x, alpha = .6,
    ...
    geom_text(aes(x = 1:.n_lvls - .2,
                  y = min(df[y]),
                  label = paste0("n=", freq, "\n"), ## could add "n=", may be busy.
                  group = {{x}}),
              data = plyr::count(df[c(x, facet)])) ## works with NULL facet.
}
...
model_ls <-
  list(a=a, ab=ab, ac=ac, ad=ad, abc=abc, abd=abd, acd=acd, abcd=abcd,
       ABcd=ABcd, ACbd=ACbd, ADcd=ADcb, ABCd=ABCd, ABDc=ABDc, ACDb=ACDb, ABCD=ABCD)
