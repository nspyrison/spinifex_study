...
.r_idx <- which(pull(model_comp_tbl[, 1]) %in% 
                  c("alpha", "alpha + beta + gamma + delta", 
...
(figEffectRange <- cowplot::plot_grid(
  ggEffectRanges,
  botRanges,
  nrow = 2))
```
...
botCis <-
  cowplot::plot_grid(ggParticipantRanges, ggSimulationRanges,
                     ncol = 2, rel_widths = c(1.07, 1))
(t_figEffectRange <- cowplot::plot_grid(
  ggExtra::ggMarginal(ggEffectRanges, type = "density", fill = "grey80"),
  ggExtra::ggMarginal(botCis,  type = "density", fill = "grey80"),
  nrow = 2))
```
