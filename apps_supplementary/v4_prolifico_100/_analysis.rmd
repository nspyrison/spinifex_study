...
my_ggpubr_facet <- function(df, x, y, color = "factor", fill = "factor", 
  ...
  .lab.y <- .45 * .y_range + max(df[y])
  ## Plot
  p <- ggboxplot(df,  x = x, y = y, fill = fill, alpha = .6,
                 palette = "Dark2",
                 add = "jitter", add.params = list(color = color, alpha = .3, width = .2)) +
    stat_compare_means(comparisons = my_comparisons, label = "p.format") +
    stat_compare_means(label.y = .lab.y) + 
