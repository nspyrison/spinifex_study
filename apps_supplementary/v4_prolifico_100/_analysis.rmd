...
dat_qual <- dat_task_agg %>% filter(`max seconds to respond` > lb, `max seconds to respond` < ub)
...
```{r}
l_u_ids <- dat_qual$participant %>% unique() %>% length() ## ~54
id_evals <- round(nrow(dat_qual) / 9, 1) ## 67.2

u_participants <- dat_qual$participant %>% unique()
l_u_participants <- u_participants  %>% length() ## ~ 52
prolific_evals <- 
  round(nrow(dat_qual[dat_qual$participant %in% u_participants, ]) / 9, 1)
paste0(l_u_participants, " unique prolific ids, with about ", prolific_evals, " evaluations after Windsorizing(.25, .95).")

## perm_num_agg
perm_num_agg <- dat_qual %>% group_by(full_perm_num, `is training`) %>%
  summarise(n = n(),
            even_studies = max(sum(`is training`) / 3, sum(!`is training`) / 6),
            cnt_participant = length(unique(participant))
  ) %>% ungroup() %>% 
  mutate(is_past_tgt = if_else(even_studies > 3, TRUE, FALSE)) %>% 
  rename(`parameter permutation number` = full_perm_num, `full evaluations` = even_studies)
...
(gg_eval_by_perm_num <- perm_num_agg %>% filter(`is training` == FALSE) %>% 
    ggplot(aes(x = `parameter permutation number`, y = `full evaluations`,
               color = `is training`, fill = `is training`)) +
...
participant_agg <- participant_agg[order(participant_agg$even_studies, decreasing = T),]
...
dat_qual_wi_participant <- dat_qual %>%
  ...
  mutate(frame_pro_id = as.integer(participant))

  ggplot(dat_qual_wi_participant, 
            aes(x = `evaluation order`, y = marks,
                frame = frame_pro_id,
                group = frame_pro_id,
                color = `is training`, fill = `is training`)
  ) +
...
dat_qual_onlyeval_renamed <- dat_qual %>% filter(`is training` == FALSE)
