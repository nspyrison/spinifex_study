...
my_ggpubr <- function(df, x = "factor", y = "marks", title = waiver(), subtitle = waiver(), facet = NULL){
  ...
  ggviolin(df, x = x, y = y, fill = x, alpha = .6,
    ...
    stat_compare_means(label.y = .lab.y,
                       aes(label = paste0("Krusal-p=", ..p.format..)), ## Global test
    ...
    my_theme +
    #loosing ggpubr content, may need to call outside.
    coord_cartesian(ylim = c(min(df[y]), max(df[y]) + .5 * .y_range)) +
    ggtitle(title, subtitle) +
    ggplot2::xlab(paste0(x, "\n(n=", nrow(df)/(.no_x_lvls * .facet_lvls), " ea.)"))
}
# my_ggpubr_with_n <- function(df, x = "factor", y = "marks", title = waiver(), subtitle = waiver(), facet = NULL){
#   .no_x_lvls <- df[x] %>% pull() %>% levels() %>% length()
#   .df_n_obs <-
#     data.frame(.x = 1:.no_x_lvls - .2,
#                .y = min(dat_qual$marks),
#                plyr::count(dat_qual, vars = c(x))) %>%
#     mutate(grp = paste0({{x}}),
#            lab = paste0(c("n=", rep("", .no_x_lvls - 1L)), freq, "\n"))
#   
#   my_ggpubr(df, x, y, title, subtitle, facet) +
#     geom_text(aes(x = .x,
#                   y = .y,
#                   label = lab, ## could add "n=", may be busy.
#                   group = grp),
#               data = .df_n_obs) ## works with NULL facet.
# }
...
see paper for latex terminology, but keep in mind:
...
ggEffectRanges <- plotREsim(REsim(ABcd)) +
  ggtitle("Effect range, CI mean marks of participants and simulations",
          "model: marks^ = \u03b1 * \u03b2 + \u03b3 + \u03b4      confidence: \u03b1 = .95") +
...
title <- cowplot::ggdraw() +
  cowplot::draw_label("Violin plot of the terms in marks^=A*B+c+d",
...
ggEffectRanges <- plotREsim(REsim(tABcd)) +
  ggtitle("Effect range, CI mean log(time) of participants and simulations",
          "model: log(time)^ = \u03b1 * \u03b2 + \u03b3 + \u03b4      confidence: \u03b1 = .95")
  theme(axis.title.x = element_blank()) +
  ylab("effect range")
...
if(F){
  ...
  ggsave("figMarksABcd.pdf", violin_ABcd, path = "./paper/figures",
         device = "pdf", width = .w, height = .w, unit = .u)
...
<!--   gc() -->
<!--   ggsave("figMarksByMarg.pdf", plot = figMarksByMarg, path = "./paper/figures", -->
<!--          device = "pdf", width = .w, height = .h * .8, unit = .u) -->
...
<!--   ggsave("figModels.pdf", plot = figModel, path = "./paper/figures", -->
<!--          device = "pdf", width = 8, height = 4, unit = "in") -->
<!-- } -->
<!-- ``` -->