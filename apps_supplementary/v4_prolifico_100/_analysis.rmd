...
dat_qual_wi_participant <- dat_qual %>%
  ...
  # tgt_ids <- head(unique(dat_qual_wi_participant$prolific_id))
  # dat_qual_wi_participant %>% 
  #   filter(prolific_id %in% tgt_ids) %>% 
  #   ggplot(aes(x = eval, y = task_marks,
  #              frame = frame_pro_id,
  #              group = frame_pro_id,
  #              color = is_training, fill = is_training)
  #   ) +
  ...
  #   scale_fill_brewer(palette = "Dark2") +
  #   geom_point(aes(size = cnt_resp), alpha = .2) +
  #   geom_line(alpha = .2) + facet_grid(vars(prolific_id))
