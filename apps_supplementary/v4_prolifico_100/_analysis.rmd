...
$$\widehat{marks} = b_{int} +
b_{SecResp} * SecResp +
b_{SecPg} * SecPg + \\
b_{RespInter} * RespInter + 
b_{InputInter} * InputInter +
b_{CntResp} * CntResp +
\epsilon
$$
Where, 
...
<!-- $$ -->
<!-- \widehat{task~marks} = b_{intercept} + \\ -->
<!-- b_{max~sec~to~respond} * max~sec~to~respond + -->
<!-- b_{response~interactions} * response~inter + -->
<!-- b_{input~interteraction} * input~inter + -->
<!-- b_{input~interteraction} -->
<!-- \epsilon -->
<!-- $$ -->
<!-- Where, $ \epsilon \sim \mathcal{N}(0,~\sigma)$ -->

Building on this, we add in all of our fixed factor terms: method factor, data dimension, variance-covariance shape, signal mixing location. Lastly we add the random effect of the participants. This random term, $effect_{participant}$, accounts for the difference between participant, that is the each individuals ability/disposition to effect their task marks. Then our mixed model is: 
...
b_{dim} * I(dim) + 
b_{shape} * I(shape) + \\
b_{location} * I(location) + 
b_{participant} * effect_{participant} +
\epsilon$$
Where,  
$$\epsilon \sim \mathcal{N}(0,~\sigma) \\
effect_{participant} \sim \mathcal{N}(0,~\sigma_{participant}) \\
factor \in (pca, grand*, radial) \\
dim \in (4, 6) \text{ variables, with 3 and 4 clusters  respectivly} \\
...
<!-- $$\widehat{task~marks} = b_{intercept} + \\ -->
<!-- b_{max~sec~to~respond} * max~sec~to~respond + -->
<!-- b_{sec~on~pg} * sec~on~pg + -->
<!-- b_{response~inter} * response~inter + -->
<!-- b_{input~inter} * input~inter + -->
<!-- b_{factor==pca} * is~factor==pca + -->
<!-- b_{factor==grand} * is~factor==grand + -->
<!-- b_{factor==radial} * is~factor==radial + \\ -->
<!-- b_{dim==4} * is~dim==4 + -->
<!-- b_{dim==6} * is~dim==6 + -->
<!-- b_{shape==eee} * is~shape==eee + -->
<!-- b_{shape==eev} * is~shape==eev + -->
<!-- b_{shape==evv} * is~shape==evv + \\ -->
<!-- b_{location==0\_1} * is~location==0\_1 + -->
<!-- b_{location==33\_66} * is~location==33\_66 + -->
<!-- b_{location==50\_50} * is~location==50\_50 + \\ -->
<!-- effect_{participant} + -->
<!-- \epsilon$$ -->
...
marks_lmer1 <- lmer(
  task_marks ~ 
    max_sec_to_resp + max_sec_on_pg + task_resp_inter + task_input_inter + cnt_resp + # Numeric
    factor + dim + shape + location + order + ## Fixed factor random terms
    (1 | participant), ## Random effect
  data = dat_qual_onlyeval_renamed
)
summary(marks_lmer1)
plotREsim(REsim(marks_lmer1))  # plot the interval estimates

print("and simplifying the model, by removing 3 of the most correlated, redundant numeric variables, we lose ~ 1% AIC relative to the first model.")
marks_lmer2 <- lmer(
  task_marks ~ 
    max_sec_to_resp + task_input_inter + cnt_resp + # Numeric
    factor + dim + shape + location + order + ## Fixed factor random terms
    (1 | participant), ## Random effect
  data = dat_qual_onlyeval_renamed
)
