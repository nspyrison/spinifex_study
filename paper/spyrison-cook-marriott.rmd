...
The visual methods are tested within-participants, with each visual being evaluated twice by each participant. The order in which experimental factors are experienced is randomized with the assignment, as illustrated in Figure \@ref(fig:figParmeterizationExample). Scatterplot matrices or parellel coordinates could alternatively be used to visualize these spaces. However we opt to focus on single biplot display to focus on the additional changes of radial tour over its most comparble visuals rather than a comprehensive comparison of visua methods. The rest of this section discusses the design standardization and unique input associated with each visual.
...
Data visualization is an integral part of understanding relationships in data and how models are fitted. When it comes to multivariate data giving a comprehensive view quickly becomes difficult as the dimensions become sizable. Analysts have the task of choosing which visualization technique to use. Because the viewing volume/time of multivariate spaces typically increase quadratically with dimensions it is important that dimension reduction is properly conducted. While there are optimization methods for static and animated visuals, the particular function used is a guided choice of the analyst.
...
There is no panecea for comprehensive visualization of multivariate spaces. We have demonstrated that there is definite value from the user-control of linear projections for variable attributions. The agency of the analyst remains an important tool for the exploratory analysis of multivariate data.
