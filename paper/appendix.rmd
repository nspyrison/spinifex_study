...
```{r, figSurveyDemographics, echo = F, out.width = '100%', fig.cap = "Heatmaps of survey participant demographics; counts of age group by completed education as faceted across preferred pronoun. Our sample tended to be between 18 and 35 years of age with an undergraduate or graduate degree."}
...
Similarly, figure \@ref(fig:figTeffectRange) shows the Gelman simulations and marginal effects of the simulation and participants for $Y_2$, the same model regressing on log response time.
...
Below we describe the \pkg{shiny} app available in the \pkg{spinifex} package which runs locally. It streamlines creating and interacting with radial tours more interactive than the application used in the user study.
...
The second tab contains interaction for selecting the manipulation variable and non-numeric columns can be used to change the color and shape of the data points in the projection. The radial tour is created in real-time animating as an interactive \pkg{plotly} .html widget. The application offers users a fast, intuitive introduction elucidating what the radial tour does and some of the features offered.
