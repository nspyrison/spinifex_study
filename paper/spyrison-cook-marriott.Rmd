...
source(rel_path("R/ggproto_pca_biplot.r"))
...
## Make ans_plot
ans_tbl <- readRDS(rel_path("../apps/spinifex_study/www/ans_tbl.rds"))
sub <- ans_tbl %>% dplyr::filter(sim_nm == tgt_sim_nm)
source(rel_path("R/pivot_longer_resp_ans_tbl.r"))
sub_longer <- pivot_longer_resp_ans_tbl(dat = sub)

source(rel_path("R/ggproto_ans_plot.r"))
...
```{r figPermuationExample, echo = F, out.width = '100%',fig.cap = "Example permuptaion selection. NEEDS CAPTION"}
