...
To recap, we have 2 primary data sets; the user study evaluations and post study survey. The former is contains the 108 trials. It's explainatory variables are marks as defined in section \ref{sec:} of the . While only 84 of these have the corresponding survey data filled out; the analysis of any survey variables is scoped accordingly.
...
## Marginal survey parameters

TODO: XXX write up here. AND FIGURE.
...
In building a set of models to test we include all single term models, a model with all independent terms. We also include an interaction term of factor by location, allowing for the slope of each location to change across each level of the factor, which is feasible. For comparison an overly complex model with many interaction terms is included. Regarding the 3 models with all terms, their model matrix is rank deficient 
