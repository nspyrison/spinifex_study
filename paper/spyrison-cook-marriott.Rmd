...
```{r, figMarksABC, echo = F, out.width = '100%', fig.cap = "Violin plots of 3 parmeter interaction terms of the model $\\widehat{marks} = \\alpha * \\beta * \\gamma$. Overlaid with global significance from the Kruskal-Wallis rank sum test, and pairwise significance from  the Wilcoxon ranked sum, both non-parametric tests. "}
if(F) ## Creation and saving figure at:
  file.edit("./apps_supplementary/v4_prolifico_100/_analysis.rmd")
...
kableExtra::kbl(model_comp_tbl_ls[[2]], align = c("l", rep("r", 7)),
             booktabs = TRUE, linesep = "", caption = "Model comparison for log(time) random effects models, all models include 2 random effect terms for participants and for simulations. We see the same trade off where the simplest factor model is perfered by AIC/BIC, while R2 and RMSE perform best with the full multiplicitative model.") %>%
  kableExtra::column_spec(column = 1, width = "3.2cm") %>%
  column_spec(column = c(2:3, 6:7), width = "1.7cm") %>%
  column_spec(column = c(4:5, 8), width = "1.1cm") %>%
  column_spec(column = c(4:5, 8), width = "1.1cm")
