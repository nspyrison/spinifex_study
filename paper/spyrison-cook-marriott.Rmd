...
```{r modelCompTable, fig.cap = "Use the caption arg in kable(), not this."}
model_comp_tbl_ls <- readRDS("./figures/tblModelCompLs.rds")
require(kableExtra)
...
```{r modelSummaryTable, fig.cap = "Use the caption arg in kable(), not this."}
ABC_coef <- readRDS("./figures/tblModelABCcoef.rds")
ABC_coef[, -6] <- round(ABC_coef[, -6], 2)
ABC_coef["df"] <- round(ABC_coef["df"], 1)
...
kableExtra::kbl(ABC_coef, booktabs = TRUE, linesep = "", caption = "TODO: XXX FILL IN.") %>%
  ...
  pack_rows("main effects", 4, 7) %>%
  pack_rows("interactions", 8, nrow(ABC_coef))
```



<!-- Residuals and random effects simulation -->
Residual plots have no noticeable non-linear trends and contain striped patterns as an artifact from regressing on discrete variables. Figure \@ref(fig:figEffectRange) illustrates (T) the effect size of the random terms participant and simulation, or more accurately, the 95\% CI from Gelman simulation of their postirior distribution. The effect size of participant is much larger than simulation. The most extreme participants are statistically significant at $\alpha$ = .95, while none of the simulation effects significantly deviate from the null of having no effect size on the marks. In comparison, (B) 95\% confidence intervals of the mean marks for participation and simulation respectively. 

```{r figEffectRange, out.width="100%", fig.show='asis', fig.cap='(T) Estimated effect ranges of the random effect terms partitcipant and data simulation of the alpha * beta * gamma model. Confidence intervals are created via Gelman simulation on the effect posterior distributions. The effect size of participant is relatively large, with several significant extrema (alpha = .95). None of the simulations deviate significantly. (B) The ordered distributions of the marks for the '}
if(F) ## Creation and saving figure at:
...
<!-- after the 12-4 meeting with Di, going to a regression oriented approach with only only violin plot of final terms in the model; boxplots scraped. -->
## Marginal block parameters and survey variables

<!-- Test: Kruskal-Wallis H test (or one-way anova) -->
...
<!-- Interpreting what marginal effects -->
Figure \@ref(fig:figMarksByMarg) (top) compares the effect of the study block parameters. Location is the inverse of what is expected, though perhaps mixing the signal diversifies the marks in that its easier to choose at least one of the correct answers of two dimensions with signal rather than the higher risk of selecting the only correct answer for all marks. EEV slightly outperforms EEE, perhaps the different shape helps distinguish the cluster or stick out from one another. Evaluation order does not have a monotonic increase. Every odd evaluation beats the next evaluation which have 4 and 6 dimensions respectively.

```{r figMarksByMarg, out.width="100%", fig.show='asis', fig.cap='Marks by the marginal effect of the block parameterizations (top) and survey variables (bottom), for the 4 variables with most support. Factor has a large degree of support for the alternative hypothesis, that use of the radial tour outperforms both the discrete benchmark, pca, and animated benchmark, grand tour for this task. The other block parameterizations quickly drop off in support. While order is near significant it is not monotonically increasing marks, the increasing difficulty of fixed block parameterization appear to have a stronger effect than any oberservable learning effect. Of the survey variables (bottom), note that the boxplots have an uneven number of observations, those with low numbers may not aproximate the true population well. Task understanding is hugely significant, though it may be conflated with latent attenion and effort of participants. Participants that default/decline recieved unanimously low marks.'}
if(F) ## Creation and saving figure at:
  file.edit("./apps_supplementary/v4_prolifico_100/_analysis.rmd")
knitr::include_graphics("./figures/figMarksByMarg.png")
```

<!-- Interpreting the effects of the marginal survey variables -->
We also test the demographic and experience question from the survey to see if they they effect marks. Figure \@ref(fig:figMarksByMarg) (bottom) show the variables with strongest evidence to explain marks. Keep in mind that some of the x groups have low counts any likely may not reflect the true distribution of their demographic. Perhaps unsurprisingly, participants that rated their task comprehension higher, averaged higher marks. Age group was narrowly significant, with the lowest, highest, and default groups scoring low. The explanatory power of age is more likely more of a proxy for attention and thoroughness rather than a true effect of age. Education correlates with marks, though is narrowly not significant at $\alpha = .95$, it is also be slight correlated with age and understanding which explain marks better then education alone. Decline/default groups received lower marks unanimously across survey questions; care should be taken to ensure or evaluate the level of effort of participants.
...
```{r modelTableSupp, fig.cap = "Use the caption arg in kable(), not this."}
model_comp_tbl_ls <- readRDS("./figures/model_comp_tbl_ls.rds")

## 2) Survey parameters
knitr::kable(model_comp_tbl_ls[[2]], align = c("l", rep("r", 7)), format="latex",
...
<!-- Residuals and random effects simulation -->
Residual plots had no noticeable non-linear trends and contain striped patterns as an artifact from regressing on discrete variables. Figure \@ref(fig:figModelSupp) illustrates a typically residual plot (left), while the right compares 95\% confidence interval of the effect ranges for participant and simulation. The effect ranges are estimated from simulations of the posterior distribution. The effect size of participant is much larger than simulation. The most extreme participants are statistically significant at $\alpha$ = .95, while none of the simulation effects significantly deviate from the null of having no effect size on the marks.

```{r figModelSupp, out.width="100%", fig.show='asis', fig.cap='Residual plot and estimated effect ranges for factor regressing on marks with random effects for both partitcipant and data simulation. The effect size of participant is relatively large, with several significant extrema (alpha = .95). None of the simulations deviate significantly.'}
if(F) ## Creation and saving figure at:
  file.edit("./apps_supplementary/v4_prolifico_100/_analysis.rmd")
