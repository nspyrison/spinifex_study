...
Given a specific dataset $\textbf{X}$ containing $k$ clusters, let $\textbf{S}_j$ be the set of observations in cluster $j$ and $\overline{S_j}$ be a vector of variable means for cluster $j$.

\begin{gather*}
  \textbf{X}_{[n,~p]} = (x_1, ~....,~ x_p)\\
  \textbf{S}_j = (s_{j1}, ~....,~ s_{ji}) ~|~ j \in [1, k],~ \textbf{S}_{j} \in \textbf{X}  \\
  \overline{S_j} = (\overline{s_1}, ~....,~ \overline{s_p})\\
\end{gather*}
...
\begin{align*}
  W &= (w_1, ~...,~ w_p) \\
  &= \frac{
    (\overline{s_{b1}} - \overline{s_{a1}}, ~...,~ \overline{s_{bp}}-\overline{ s_{ap}})
  }{
    \sum_{i=1}^{p}(|\overline{s_{bi}} - \overline{s_{ai}}|)
  } - \frac{1}{p}
\end{align*}
...
\begin{align*}
  M &= (m_1, ~...,~ m_p) \\
  &= I(r_i) * sign(w_i) * \sqrt{|w_i|}
\end{align*}

Where $I$ is the indicator function. Then the total marks for this task is the sum of the marks vector.

```{r, figBiplotScoring, echo = F, out.width = '100%', fig.cap = "PCA Biplot with Score and weight evaluation. TODO XXX NEEDS CAPTION"}
knitr::include_graphics("./figures/figBiplotScoring.png")
```

_TODO: XXX, NEED caption and title for figure_
...
<!-- discuss training, response text, timed evaluations -->
_TODO: XXX, discuss untimed training, response text, timed evaluations_


## Factor application {#sec:factors}

<!-- Background for methodology, application here -->
Section \ref{sec:background} gives the sources and a description of the visual factors PCA, grand tours, and radial manual tours. Below we cover the differences in the application and parameterization applied in the user study. Due to physically distancing from COVID-19 what was originally intended to be run in person with study invigilator had to be simplified to be understood and usable in a crowdsourcing application. We opted for percomputed images and animations in order to simplify input interactions and improve user experience.
...
<!-- Training, eval, eval -->
Within each factor there is one training trial, followed by 2 time-limited evaluation trials. The training allows the participant to become familiar with the inputs and visual specific to the factor. Upon clicking the proceed button text containing the correct answer displays with visual still intact to explore further. Training trial always happen with the simplest parameterization and are followed by 2 evaluations of increasingly difficult parameters which are limited to 1 minute each.
