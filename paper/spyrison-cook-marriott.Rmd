...
bold_column_extrema <- function(df, col, extrema = max){ 
  ## c("*", "**", "~~"), c("italics", "bold", "strikethrough")
  v <- as.data.frame(df[, col])[, 1]
  ## Make sure values are not factors
  r_idx <- which(v == extrema(v))
  ## Update formatting
  kableExtra::column_spec(df, column = c(r_idx, col), bold = T)
}
# .v <- model_comp_tbl_ls[[1]][,4]
# model_comp_tbl_ls[[1]][,4] = kableExtra::cell_spec(.v, bold = ifelse(.v == min(.v), T, F))
...
kableExtra::kbl(model_comp_tbl_ls[[1]], align = c("l", rep("r", 7)),
  ...
  kableExtra::column_spec(column = c(4:5, 8), width = "1.1cm")
  # 
  # bold_column_extrema(4, min) %>% 
  # bold_column_extrema(5, min) %>%
  # bold_column_extrema(6, max) %>%
  # bold_column_extrema(7, max) %>%
  # bold_column_extrema(8, max)
