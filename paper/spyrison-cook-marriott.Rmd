...
abstract: >
  Principal component analysis is the classical standard for viewing projections of multivariate spaces. However, the full story of the data is rarely portrayed accurately in a few projections. More recently, grand tours offer an animation of random walks offering many angles to view embedded spaces. A manual tour provides a means of controlling the contribution of individual variables to a projected subspace. We run a within-participants user study to compare and contrast the efficacy of these 3 visualizations. Additionally, we have developed an application to facilitate the exploration of multivariate data through the use of various tour methods. Crowdsourced participants (n=XXX, via prolific.co) were asked to identify which variable(s) attribute the difference between 2 clusters of data. The scope of the synthetic data varied across location, variance-covariance shape, number of dimensions, and number of clusters contained in the data. Accuracy, speed, subjective evaluation, and demographic data are collected. User feedback suggests that...
