---
title: "The effect of user interaction for understanding variable contributions to
  structure in linear projections"
...
abstract: |
  Principal component analysis and other eigenvalue decomposition enjoy wide when it comes to visualizing multivariate spaces. The grand tour is adds the continity of near-by linear projections while moving towards a randomly selected basis. The radial tours keep this continuity of projection space while manipulating the contribution of a single variable. However, there is an absence of work evaluating the efficacy of different multivariate visualizations. We perform a between-participant user study evaluating these visualizations: biplots of principal componts, grand tours, and radial tours. We used accuracy and speed as evaluation measures for a task of supervised clusters where participants indicate which variables contain the seperation between two target clusters. Prolific.co is used to crowdsourced N=108 participants across 648 task evaluations. Additionaly, 84 post-study surveys were collected. We measure accross 3 dimensions of block parameterizations: dimensionality, location, and shape. In summary, we find that radial tours score higher and respond slightly faster than alternatives. The effect of the vizualization to dwarf the effect of the other block parameterizations. The effect of the visual is itself dwarfed by the random effect of the individual participant.
---
\bibliography{spyrison-cook-marriott}

<!-- JDSSV Sweave example -->
...
<!-- \Plaintitle{Guidelines for the Journal of Data Science, Statistics, and Visualisation Version 0.1} -->
<!-- \Shorttitle{JDSSV Guidelines, Version 0.1} -->
...
knitr::include_graphics("./figures/imageName.png") ## Note relative to the folder that the .rmd is in not the project dir()
```
...
$~~~~~~H_0: \text{visualization factor does \textit{not} impact task performance} \\$
...
&\alpha * \beta& \widehat{marks} =& \mu + \alpha_i * \beta_j + \textbf{Z} + \textbf{W} + \epsilon \\
...
rownames(coef_ls[[1]]) <- rownames(coef_ls[[2]]) <-
  c("(Intercept)"                   ,"fct=grand"                    ,"fct=radial"
    ...
    ,"shp=ban"                      ,"fct=grand:loc=33_66"          ,"fct=radial:loc=33_66"
    ,"fct=grand:loc=50_50"          ,"fct=radial:loc=50_50"         ,"fct=grand:shp=EEV"
    ,"fct=radial:shp=EEV"           ,"fct=grand:shp=ban"            ,"fct=radial:shp=ban"
    ,"loc=33_66:shp=EEV"            ,"loc=50_50:shp=EEV"            ,"loc=33_66:shp=ban"
    ,"loc=50_50:shp=ban"            ,"fct=grand:loc=33_66:shp=EEV"  ,"fct=radial:loc=33_66:shp=EEV"
    ,"fct=grand:loc=50_50:shp=EEV"  ,"fct=radial:loc=50_50:shp=EEV" ,"fct=grand:loc=33_66:shp=ban"
    ,"fct=radial:loc=33_66:shp=ban" ,"fct=grand:loc=50_50:shp=ban"  ,"fct=radial:loc=50_50:shp=ban")
...
kableExtra::kbl(coef_ls[[1]], booktabs = TRUE, linesep = "", format = "latex",
                caption = "The model coeffients for $\\widehat{marks} = \\alpha * \\beta * \\gamma$, with factor:pca, locaation0/100, and shapeEEE held as baselines. Factor being radial is the fixed term with the strongest evidence in support of the hypothesis. Evidence for the interaction terms vary widely. The evidence in favor of any one coeffcient is relatively modest, this is due inpart to expending the fixed terms explainatory across so many interaction terms; each fixed term appears much more extreme models with less terms.") %>%
  kableExtra::column_spec(column = 1, width = "5.5cm") %>%
...
We also want to visually explore the conditional variables in the model. Figure \@ref(fig:figMarksABC) explores violin plots of marks by factor while faceting on the location (vertical) and shape (horizontal). Notice that the conditional variable regularly have much smaller p-value, with seemingly so much more evidence in their favor, this is because they are not "diversifying" their explanatory power across numerous interaction terms, so to speak.

```{r, figMarksABC, echo = F, out.width = '100%', fig.cap = "Violin plots of 3 parmeter interaction terms of the model $\\widehat{marks} = \\alpha * \\beta * \\gamma$. Overlaid with global significance from the Kruskal-Wallis test, and pairwise significance from the Wilcoxon test, both non-parametric, ranked sum tests suitable for handling discrete data. Participants are more confident and find the radial easier to use relative to the grand tour. Particpants claim low familiarity as we would expected fro crowdsourced particiants. Radial is more preffered compared with either alternative for this task."}
if(F) ## Creation and saving figure at:
  file.edit("./apps_supplementary/v4_prolifico_100/_analysis.rmd")
...
if(F) ## Creation and saving figure at:
  file.edit("./paper/R/fig_population_demographics.r")
knitr::include_graphics("./figures/figSubjectiveMeasures_w.violin_hori.png") 
## Alternatively use:
